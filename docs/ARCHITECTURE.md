...
3. **Mundo permanente com background substituível**: o `ImageBackground` da Tela Mundo escolhe entre `mundo_terreno_vazio.png` (v1), `background_mundo_v2.png` (v2 — desbloqueado no Nível 5) e `background_mundo_v3.png` (v3 — desbloqueado no Nível 8) baseado em `WORLD_ELEMENTS`. O Nível 10 fará uma terceira substituição radical (background árido). Padrão: `{ add: "background_mundo_vN", replaces: "background_mundo_vN-1" }` na reward do nível.
...
8. **Variáveis no programa = `player.inventory.fruits` (Nível 8)**: o MVP introduziu o conceito de "variável" no Nível 8 reaproveitando o campo `inventory.fruits` do `PlayerState` em vez de criar um campo genérico `variables: Record<string, number>` no `WorldState`. Decisão YAGNI: enquanto há um único caso de variável no MVP, generalizar é especulação. Reset automático via clone JSON do `initialWorld` no `resetWorld`. Quando aparecer 2º ou 3º caso (provavelmente pós-MVP), a generalização vira refactor objetivo. Ver DECISIONS.md.
9. **Padrão "elemento que migra pro background" (Nível 8)**: a partir do Nível 8, o `app/world.tsx` adota uma lógica de supressão condicional de elementos do primeiro plano. Quando `worldElements` contém o flag `background_mundo_v3`, a árvore principal (`fruit_tree_lvl7`) e as 3 mini-árvores (`mini_tree_lvl6_a/b/c`) DEIXAM de renderizar no primeiro plano — visualmente "migram" pro background v3 (que tem essas árvores integradas na composição). Implementação: cada `setShow*` ganha `&& !hasBgV3` no encadeamento. Aditivo, preserva os Níveis 1-7. Cria sensação de profundidade temporal: "o cuidado individual virou paisagem".
10. **Cadeia da planta principal estendida** (`seed_lvl1 → sprout_lvl2 → grown_sprout_lvl3 → mini_tree_lvl4 → young_tree_lvl5 → fruit_tree_lvl7 → background v3`): a cadeia chega ao seu fim no Nível 7. No Nível 8, o último elo (`fruit_tree_lvl7`) é absorvido pelo background, encerrando a sequência de "planta crescendo no primeiro plano". Padrão narrativo: elementos "fazem o ciclo" — entram, evoluem, integram-se à paisagem.
11. **Mascote-Gabarito — execução em duas passadas (Níveis 7+)**: a partir do Nível 7, vencer um nível dispara uma segunda execução do interpretador, agora com a solução ótima (`optimalSolution` na config do nível), executada pelo mascote. É uma feature **aditiva-retroativa**: não altera a mecânica dos níveis 7-8, só adiciona uma cena posterior ao sucesso (equivalente a uma animação no level summary). Implicações arquiteturais: (a) o `LevelScene` ganhou um conceito de **executor** (`"avatar" | "mascote"`) que troca só o sprite renderizado, sem tocar na mecânica de movimento; (b) o `WorldState` é **resetado entre as duas execuções** via clone JSON do `initialWorld` — a planta cresceu na passada da criança, volta ao estado inicial pra passada do mascote; (c) a `optimalSolution` usa o tipo `OptimalSolutionBlock` (sem `id`), convertido em `ProgramBlock` com ids estáveis em runtime — mantém `lib/` sem depender de `components/`. Níveis 1-6 não definem `optimalSolution` e ignoram toda a feature. Fundamento narrativo (mascote como segundo agente moral, fundação pra quebra de expectativa no Nível 9) registrado em DECISIONS.md.

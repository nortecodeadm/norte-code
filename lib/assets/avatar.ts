/**
 * Avatar asset mapping — componentized layer system
 * 
 * The avatar is composed of 3 layers rendered at runtime:
 * 1. Body (skinTone) — base layer
 * 2. Outfit (outfit) — overlays the white t-shirt
 * 3. Hair (hairStyle + hairColor) — top layer
 * 
 * All assets are 2048x2048 with same canvas anchoring.
 * Total combinations: 4 skins × 4 styles × 4 colors × 3 outfits = 192
 */
...
export function getHairAsset(style: HairStyle, color: HairColor) {
  // Map the style name to the file naming convention (without special chars)
  const styleKey = style;
  const key: HairKey = `${styleKey}_${color}`;
